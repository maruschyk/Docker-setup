...
export class ProductsService {
  ...
  private readonly productsCacheKeys = new Set<string>();

  private async clearProductsCache(): Promise<void> {
    await Promise.all(
      [...this.productsCacheKeys].map((key) => this.cacheManager.del(key)),
    );

    this.productsCacheKeys.clear();
  }
  async findAll(query: ProductQueryDto) {
    ...
    this.productsCacheKeys.add(cacheKey);
    return result;
  }
